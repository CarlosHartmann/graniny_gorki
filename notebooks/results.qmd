...
# LLMs
from process_raw_data import commercial_llm_responses, opensource_llm_responses
...
all_llms = list(accuracy_dict_commercial_llms.items()) + list(accuracy_dict_opensource_llms.items())
...
top_3_llms = sorted(
    top_3_commercial_llms + top_3_opensource_llms,
    key=lambda x: x[1],
    reverse=True
)[:3]
...
ax3.bar(
    ...
    oss_values,
    yerr=oss_err,
    color=oss_palette[:len(oss_labels)],  # Updated to use new orange palette
    edgecolor="black",
    linewidth=1.2,
    error_kw={"capsize": 5, "elinewidth": 1.2, "ecolor": "black"}
)
